...
class PendingTextTensorQueue:
    """FIFO queue backed by GPU tensor chunks plus a head cursor.

    The talker consumes one future text row per decode step. Incoming tensors
    stay on device and are appended as chunks, avoiding both row-wise D2H/H2D
    transfers and repeated concatenation of unconsumed rows.
    """

    # Keep the public ``rows`` attribute as the current head chunk for callers
    # that inspect its device/dtype. Additional chunks wait in ``_chunks``.
    ...
    def __getitem__(self, idx: int) -> torch.Tensor:
        ...
        if idx < 0:
            idx += len(self)
        if self.rows is None or idx < 0 or idx >= len(self):
            raise IndexError(idx)

        head_remaining = int(self.rows.shape[0]) - self.cursor
        if idx < head_remaining:
            return self.rows[self.cursor + idx]
        idx -= head_remaining
        for chunk in self._chunks:
